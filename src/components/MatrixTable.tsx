...
import { MatrixConfig, MatrixRow, SgkBook, PpctDataset } from '../types';
import {
  calculateTopicPointSummary,
  getPpctReferenceBreakdown,
...
export const MatrixTable: React.FC<MatrixTableProps> = React.memo(({
  ...
  const [showPpctReference, setShowPpctReference] = useState<boolean>(false);
  ...
  const ppctReference = activePpct
    ? getPpctReferenceBreakdown(activePpct, {
        limitWeekFrom: config.limitWeekFrom,
        limitWeekTo: config.limitWeekTo,
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-xs mb-8">
      ...
      <div className="border-b border-slate-200 bg-white">
        <div className="px-4 py-2.5 flex flex-wrap items-center justify-between gap-2 bg-slate-50/70 border-b border-slate-200">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] text-slate-500 hidden sm:inline">
              (Công thức: Điểm = (Số tiết × 10) / Tổng số tiết)
            </span>
          </div>

          <div className="flex items-center gap-2">
            {/* Quick matrixGroupBy mode switcher */}
            {onUpdateConfig && (
              <div className="flex items-center gap-1 bg-white border border-slate-300 rounded-lg p-0.5 text-[11px]">
                <button
                  type="button"
                  onClick={() => {
                    if (config.matrixGroupBy !== 'chapter') {
                      onUpdateConfig({ matrixGroupBy: 'chapter' });
                      if (onRegenerateMatrix) setTimeout(onRegenerateMatrix, 50);
                    }
                  }}
                  ...
                  title="Gom các bài học theo từng Chủ đề / Chương lớn (Chuẩn khung công văn Bộ GD&ĐT)"
                >
                  Khung Chủ đề lớn
                </button>
                <button
                  ...
                  onClick={() => {
                    if (config.matrixGroupBy !== 'lesson') {
                      onUpdateConfig({ matrixGroupBy: 'lesson' });
                      if (onRegenerateMatrix) setTimeout(onRegenerateMatrix, 50);
                    }
                  }}
                  className={`px-2 py-0.5 rounded font-medium transition-colors ${
                  ...
                  title="Tách riêng từng bài học thành từng dòng ma trận"
                >
                  Chi tiết từng bài
                </button>
              </div>
            ...
            <button
              ...
              className="flex items-center gap-1 text-slate-600 hover:text-slate-900 text-[11px] font-medium px-2 py-1"
            >
        ...
        {showTopicCalc && (
          <div className="p-4 space-y-3">
              ...
              <table className="w-full text-xs text-slate-800 border border-slate-300 border-collapse">
                <thead>
                  <tr className="bg-slate-100 text-slate-800 font-semibold border-b border-slate-300">
                    <th className="py-2 px-3 text-left border-r border-slate-300 w-44 bg-slate-100/90">
                      Chủ đề / Bài học
                    </th>
                    {topicSummary.items.map((item) => (
                      <th key={item.topicIndex} className="py-2 px-2 text-center border-r border-slate-300 min-w-[120px]">
                        <div className="font-bold text-slate-900">Chủ đề {item.topicIndex}</div>
                        <div className="text-[10px] font-normal text-slate-500 line-clamp-1 mt-0.5" title={item.topicName}>
                          {item.topicName}
                        </div>
                      </th>
                    ))}
                ...
                <tbody>
                  <tr className="border-b border-slate-200 bg-white">
                    <td className="py-1.5 px-3 font-medium border-r border-slate-300 bg-slate-50 text-slate-700">
                      Số lượng bài học
                    </td>
                    {topicSummary.items.map((item) => (
                      <td key={item.topicIndex} className="py-1.5 px-2 text-center border-r border-slate-300">
                        <span className="font-semibold text-emerald-900 bg-emerald-50 px-2 py-0.5 rounded text-[11px]">
                          {item.lessonsCount ? `${item.lessonsCount} bài` : '1 bài'}
                        </span>
                      </td>
                    ))}
                    <td className="py-1.5 px-2 text-center font-bold bg-slate-100 text-slate-700">
                      {topicSummary.items.reduce((sum, it) => sum + (it.lessonsCount || 1), 0)} bài
                    </td>
                  </tr>
                  <tr className="border-b border-slate-200 bg-white">
                    <td className="py-1.5 px-3 font-medium border-r border-slate-300 bg-slate-50 text-slate-700">
                    ...
                    {topicSummary.items.map((item) => (
                      <td key={item.topicIndex} className="py-1.5 px-2 text-center border-r border-slate-300 font-bold text-slate-900">
                        {item.periods} tiết
                      </td>
                    ))}
                    <td className="py-1.5 px-2 text-center font-bold bg-slate-100 text-slate-900">{topicSummary.totalPeriods} tiết</td>
                  </tr>
                  <tr className="border-b border-slate-200 bg-white">
                    <td className="py-1.5 px-3 font-medium border-r border-slate-300 bg-slate-50 text-slate-700">
                      Tỉ lệ thời lượng (%)
                    ...
                    {topicSummary.items.map((item) => {
                      const pct = item.pctThoiLuong || Math.round((item.periods / (topicSummary.totalPeriods || 1)) * 100);
                      return (
                        <td key={item.topicIndex} className="py-1.5 px-2 text-center border-r border-slate-300 font-semibold text-blue-700">
                          {pct}%
                        </td>
                      );
                    })}
                    <td className="py-1.5 px-2 text-center font-bold bg-slate-100 text-blue-900">100%</td>
                  </tr>
                  <tr className="border-b border-slate-200 text-slate-600 bg-white">
                    <td className="py-1.5 px-3 font-medium border-r border-slate-300 bg-slate-50">
                      Điểm (chưa làm tròn)
                    </td>
                    {topicSummary.items.map((item) => (
                  ...
                  <tr className="bg-emerald-50/60 font-semibold text-emerald-900">
                    ...
                    {topicSummary.items.map((item) => (
                      <td key={item.topicIndex} className="py-1.5 px-2 text-center border-r border-slate-300 font-bold text-emerald-800 text-sm">
                        {item.roundedScore.toString().replace('.', ',')}
                      </td>
                    ))}
                    <td className="py-1.5 px-2 text-center font-extrabold bg-emerald-100 text-emerald-950 text-sm">
            ...
            <p className="text-[11px] text-slate-500 italic">
              * Điểm mỗi chủ đề được hệ thống tự động tính toán dựa trên số tiết đã học theo công thức chuẩn: <strong>Điểm = (Số tiết × 10) / Tổng số tiết</strong> để cân đối số lượng câu hỏi trắc nghiệm và tự luận chính xác tuyệt đối.
            </p>

            {/* PPCT Reference Detail Accordion */}
            {showPpctReference && ppctReference.length > 0 && (
              <div className="mt-3 p-3 bg-slate-50 border border-slate-200 rounded-lg space-y-2.5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5 text-xs font-bold text-emerald-950">
                    <BookOpen className="w-4 h-4 text-emerald-700" />
                    <span>Chi tiết đối chiếu phân phối chương trình (PPCT) bám sát SGK:</span>
                  </div>
                  <span className="text-[11px] text-slate-500">
                    Tổng cộng: {ppctReference.reduce((s, r) => s + r.lessonsCount, 0)} bài ({ppctReference.reduce((s, r) => s + r.totalPeriods, 0)} tiết)
                  </span>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
                  {ppctReference.map((ref) => (
                    <div key={ref.topicIndex} className="bg-white p-2.5 rounded border border-slate-200 text-xs shadow-2xs">
                      <div className="flex items-start justify-between gap-2 mb-1.5 border-b border-slate-100 pb-1">
                        <div>
                          <div className="font-bold text-slate-900 text-[11.5px]">
                            Chủ đề {ref.topicIndex}: {ref.chapterName}
                          </div>
                          <div className="text-[10.5px] text-slate-500">
                            {ref.weeksSpan} • {ref.lessonsCount} bài • {ref.totalPeriods} tiết ({ref.pctThoiLuong}% thời lượng)
                          </div>
                        </div>
                        <span className="bg-emerald-100 text-emerald-900 font-bold px-2 py-0.5 rounded text-[11px] shrink-0">
                          {ref.roundedScore.toString().replace('.', ',')} điểm
                        </span>
                      </div>

                      <div className="space-y-1 pl-1">
                        {ref.lessons.map((ls, lIdx) => (
                          <div key={lIdx} className="flex items-center justify-between text-[11px] text-slate-700">
                            <span className="line-clamp-1 flex-1 pr-2">
                              • {ls.name}
                            </span>
                            <span className="text-slate-500 font-medium shrink-0">
                              {ls.periods} tiết (T{ls.week})
                            </span>
                          </div>
                        ))}
                      </div>
